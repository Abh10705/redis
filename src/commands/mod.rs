...
pub fn handle_ping(_args: &[String]) -> String { encode_simple_string("PONG") }
pub fn handle_echo(args: &[String]) -> String { if args.len() < 2 { encode_error("wrong number of arguments for 'echo' command") } else { encode_bulk_string(&args[1]) } }
pub fn handle_get(args: &[String], db: &mut InMemoryDB) -> String { if args.len() != 2 { encode_error("wrong number of arguments for 'get' command") } else { match db.get(&args[1]) { Ok(Some(val)) => encode_bulk_string(&val), Ok(None) => encode_null_bulk_string(), Err(msg) => encode_error(msg), } } }
pub fn handle_config(args: &[String], config: &Arc<Config>) -> String { if args.len() < 3 || args[1].to_uppercase() != "GET" { encode_error("Only CONFIG GET is supported") } else { match args[2].as_str() { "dir" => encode_array(&["dir".to_string(), config.dir.clone()]), "dbfilename" => encode_array(&["dbfilename".to_string(), config.dbfilename.clone()]), _ => encode_error("Unknown CONFIG key"), } } }
pub fn handle_keys(args: &[String], db: &mut InMemoryDB) -> String { if args.len() == 2 && args[1] == "*" { encode_array(&db.keys()) } else { encode_error("Only KEYS * is supported") } }
pub fn handle_llen(args: &[String], db: &mut InMemoryDB) -> String { if args.len() != 2 { encode_error("wrong number of arguments for 'llen' command") } else { match db.llen(&args[1]) { Ok(list_len) => encode_integer(list_len as i64), Err(msg) => encode_error(msg), } } }
pub fn handle_lrange(args: &[String], db: &mut InMemoryDB) -> String { if args.len() != 4 { encode_error("wrong number of arguments for 'lrange' command") } else { let start_res = args[2].parse::<isize>(); let stop_res = args[3].parse::<isize>(); if start_res.is_err() || stop_res.is_err() { encode_error("value is not an integer or out of range") } else { match db.lrange(&args[1], start_res.unwrap(), stop_res.unwrap()) { Ok(elements) => encode_array(&elements), Err(msg) => encode_error(msg), } } } }
pub fn handle_info(args: &[String], state: &ServerState) -> String { if args.len() > 1 && args[1].to_lowercase() == "replication" { let info = format!( "role:{}\r\nmaster_replid:{}\r\nmaster_repl_offset:{}", state.role, state.master_replid, state.master_repl_offset ); encode_bulk_string(&info) } else { encode_bulk_string("") } }
...
pub fn handle_blpop( args: &[String], db_arc: &Arc<Mutex<InMemoryDB>>, notifier: &Arc<Mutex<Notifier>>) -> String {
    if args.len() != 3 { return encode_error("wrong number of arguments for 'blpop' command"); }
    let key = args[1].clone();
    let timeout_res = args[2].parse::<f64>();
    if timeout_res.is_err() { return encode_error("timeout is not a float or out of range"); }
    ...
    loop {
        ...
        if let Ok(Some(element)) = db_lock.lpop(&key) {
            return encode_array(&[key, element]);
        }
...
pub fn handle_psync( args: &[String], state: &ServerState, stream: &mut TcpStream, propagator: &mut CommandPropagator, ) -> std::io::Result<()> {
